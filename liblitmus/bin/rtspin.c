...
int main(int argc, char** argv)
{
	...
	lt_t wcet_hi;
	lt_t period;
	double wcet_ms, wcet_hi_ms, period_ms, deadline_ms;
	...
	printf("\nCrit : %d, c_lo : %.2fms, c_hi : %.2fms, period : %.2fms, deadline :%.2fms"
	" priority : %u\n",crit, wcet_ms, wcet_hi_ms, period_ms, deadline_ms, priority);

	init_rt_task_param(&param);
	param.exec_cost = wcet;
	param.exec_cost_hi = wcet_hi;
	param.relative_deadline = deadline;
	param.relative_deadline_low = deadline; //will be modified inside kernel
	param.crit = crit;
	param.period = period;
